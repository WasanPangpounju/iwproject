...
import PieChart from "@/app/su/components/ChartDisabled";
...
function SupervisorPage() {
  ...
  const count_graduation = allUser?.filter(
    (user) => user?.typePerson === "บัณฑิตพิการ"
  )?.length;
  const count_students = allUser?.filter(
  ...
  //level
  const count_level1 = allEducations?.filter(
    (edu) => edu.level[0] === "1"
  )?.length;
  const count_level2 = allEducations?.filter(
    (edu) => edu.level[0] === "2"
  )?.length;
  const count_level3 = allEducations?.filter(
    (edu) => edu.level[0] === "3"
  )?.length;
  const count_level4 = allEducations?.filter(
    (edu) => edu.level[0] === "4"
  )?.length;
