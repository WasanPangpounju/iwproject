...
function AdminPage() {
    ...
    const tempAllUser = allUser?.filter((user) => user?.university === dataUser?.university && user?.role === "user");
    const tempAllEducations = allEducations?.filter((edu) => edu?.university?.some((e) => e === dataUser?.university));
    console.log(tempAllEducations)
    //catalog_user
    const count_allUser = tempAllUser?.length;
    const count_graduation = tempAllUser?.filter((user) => user?.typePerson === "บัณฑิตพิการ")?.length;
    const count_students = tempAllUser?.filter((user) => user?.typePerson === "นักศึกษาพิการ")?.length;
    const count_male = tempAllUser?.filter((user) => user?.sex === "ชาย")?.length;
    const count_female = tempAllUser?.filter((user) => user?.sex === "หญิง")?.length;

    //count disabled
    const count_d1 = tempAllUser?.filter((user) =>
        user?.typeDisabled?.some((disa) => disa === "พิการทางการมองเห็น")
    )?.length;
    const count_d2 = tempAllUser?.filter((user) =>
        user?.typeDisabled?.some((disa) => disa === "พิการทางการได้ยินหรือสื่อความหมาย")
    )?.length;
    const count_d3 = tempAllUser?.filter((user) =>
        user?.typeDisabled?.some((disa) => disa === "พิการทางการเคลื่อนไหวหรือทางร่างกาย")
    )?.length;
    const count_d4 = tempAllUser?.filter((user) =>
        user?.typeDisabled?.some((disa) => disa === "พิการทางจิตใจหรือพฤติกรรม")
    )?.length;
    const count_d5 = tempAllUser?.filter((user) =>
        user?.typeDisabled?.some((disa) => disa === "พิการทางสติปัญญา")
    )?.length;
    const count_d6 = tempAllUser?.filter((user) =>
        user?.typeDisabled?.some((disa) => disa === "พิการทางการเรียนรู้")
    )?.length;
    const count_d7 = tempAllUser?.filter((user) =>
        user?.typeDisabled?.some((disa) => disa === "พิการทางการออทิสติก")
    )?.length;

    //level
    const count_level1 = tempAllEducations?.filter((edu) => edu.level[0] === '1')?.length;
    const count_level2 = tempAllEducations?.filter((edu) => edu.level[0] === '2')?.length;
    const count_level3 = tempAllEducations?.filter((edu) => edu.level[0] === '3')?.length;
    const count_level4 = tempAllEducations?.filter((edu) => edu.level[0] === '4')?.length;

    //student age
    const currentYear = new Date().getFullYear();
    const count_age11_20 = tempAllUser?.filter((user) => {
        const birthYear = Number(user?.yearBirthday)
        const age = currentYear - birthYear;
        return age >= 11 && age <= 20;
    })?.length;
    const count_age21_30 = tempAllUser?.filter((user) => {
        const birthYear = Number(user?.yearBirthday)
        const age = currentYear - birthYear;
        return age >= 21 && age <= 30;
    })?.length;
    const count_age31_40 = tempAllUser?.filter((user) => {
        const birthYear = Number(user?.yearBirthday)
        const age = currentYear - birthYear;
        return age >= 31 && age <= 40;
    })?.length;
    const count_age41_50 = tempAllUser?.filter((user) => {
        const birthYear = Number(user?.yearBirthday)
        const age = currentYear - birthYear;
        return age >= 41 && age <= 50;
    })?.length;
    const count_age50 = tempAllUser?.filter((user) => {
        const birthYear = Number(user?.yearBirthday)
        const age = currentYear - birthYear;
        return age > 50;
    })?.length;
