...
function AdminPage() {
  ...
  const tempAllUser = allUser?.filter(
    (user) => user?.university === dataUser?.university && user?.role === "user"
  );
  const tempAllEducations = allEducations?.filter((edu) =>
    edu?.university?.some((e) => e === dataUser?.university)
  );
  console.log(tempAllEducations);
  //catalog_user
  const count_allUser = tempAllUser?.length;
  const count_graduation = tempAllUser?.filter(
    (user) => user?.typePerson === "บัณฑิตพิการ"
  )?.length;
  const count_students = tempAllUser?.filter(
    (user) => user?.typePerson === "นักศึกษาพิการ"
  )?.length;
  const count_male = tempAllUser?.filter((user) => user?.sex === "ชาย")?.length;
  const count_female = tempAllUser?.filter(
    (user) => user?.sex === "หญิง"
  )?.length;
  ...
  //level
  const count_level1 = tempAllEducations?.filter(
    (edu) => edu.level[0] === "1"
  )?.length;
  const count_level2 = tempAllEducations?.filter(
    (edu) => edu.level[0] === "2"
  )?.length;
  const count_level3 = tempAllEducations?.filter(
    (edu) => edu.level[0] === "3"
  )?.length;
  const count_level4 = tempAllEducations?.filter(
    (edu) => edu.level[0] === "4"
  )?.length;
