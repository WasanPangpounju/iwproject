...
function StudentSkills({ dataUser, id }) {
    ...
    const handleTrainDocument = (event, index) => {
        ...
        if (selectedFile) {
            ...
            const storageRef = ref(storage, `users/documents/trainHistory/${dataUser?.email}/${fileName}`);
    ...
    return (
        <div className='mt-5'>
            {dataSkills ? (
                <form onSubmit={(e) => handleSubmit(e, skills.length, trains.length)} className={`${bgColorMain2} ${bgColor} rounded-lg flex flex-col gap-7`}>
                    <div>
